 ...
 */
    const dynamoGet = async (tableName: string, key: DocumentClient.Key, consistentRead: boolean = true): Promise<GetItemOutput> => {
 ...
 * @returns {Promise<any>}
 */
    const dynamoQuery = async (tableName: string, partitionKey: string, value: any, asc: boolean = true, consistentRead: boolean = true): Promise<any> => {
 ...
 * @returns {Promise<any>}
 */
    const dynamoQuerySecondaryIndex = async (tableName: string, indexName: string, partitionKey: string, value: string | number, asc: boolean = true): Promise<any> => {
        const params: aws.DynamoDB.DocumentClient.QueryInput = {
            TableName: tableName,
            IndexName: indexName,
            KeyConditionExpression: "#hash = :v",
 ...
 * @returns {Promise<any>}
 */
    const dynamoPut = async (tableName: string, item: aws.DynamoDB.DocumentClient.PutItemInputAttributeMap): Promise<any> => {
 ...
 * @returns {Promise<any>}
 */
    const dynamoUpdate = async (tableName: string, key: aws.DynamoDB.DocumentClient.Key, name: string, value: any): Promise<any> => {
        const currentTime: string = await commonFunction.getCurrentTime();
