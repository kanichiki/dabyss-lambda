...
export class Craziness {
    ...
    static async getCrazinessIdsMatchType(type: number): Promise<number[]> {
        const secondaryIndex: string = "type-craziness_id-index";
        const crazinessIds: number[] = [];
        for (let i = 1; i <= type; i++) {
            const data: DocumentClient.QueryOutput = await dabyss.dynamoQuerySecondaryIndex(crazinessTable, secondaryIndex, "type", i);
