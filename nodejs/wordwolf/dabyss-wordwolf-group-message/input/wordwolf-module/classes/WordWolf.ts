import * as aws from "../../dabyss-module/clients/awsClient";

import { Group } from '../../dabyss-module/classes/Group';
import { Game } from '../../dabyss-module/classes/Game';
 ...
 * @extends {Game}
 */
export class WordWolf extends Game {
    wordSetTable: string;
    settingNames: string[];
    ...
    async getWordSetIdsMatchDepth(depth: number): Promise<number[]> {
        ...
        if (depth != 3) {
            const data: DocumentClient.QueryOutput = await aws.dynamoQuerySecondaryIndex(this.wordSetTable, index, "depth", Number(depth));
            if (data.Items != undefined) {
            ...
            const data3: DocumentClient.QueryOutput = await aws.dynamoQuerySecondaryIndex(this.wordSetTable, index, "depth", 3);
            if (data3.Items != undefined) {
                for (let item of data3.Items) {
                    wordSetIds.push(item.word_set_id);
                }
            }
            const data4: DocumentClient.QueryOutput = await aws.dynamoQuerySecondaryIndex(this.wordSetTable, index, "depth", 4);
    ...
    async getWordSet(): Promise<DocumentClient.AttributeMap> {
        ...
        const data: DocumentClient.GetItemOutput = await aws.dynamoGet(this.wordSetTable, key);
    ...
    async updateWordSet(depth: number): Promise<void> {
        try {
            ...
            const isReverse: boolean = await commonFunction.getRandomBoolean();
    ...
    async chooseWolfIndexes(wolfNumber: number): Promise<number[]> {
        ...
        const wolfIndexes: number[] = await commonFunction.chooseRandomIndexes(userNumber, wolfNumber);
        return wolfIndexes;
    }
    ...
    async getWolfNumberOptions(): Promise<number[]> {
        const userNumber: number = await this.getUserNumber();
        const maxWolfNumber: number = await commonFunction.calculateMaxNumberLessThanHalf(userNumber);
    ...
    async chooseLunaticIndexes(lunaticNumber: number): Promise<number[]> {
        ...
        const lunaticIndexes: number[] = await commonFunction.chooseRandomIndexes(userNumber, lunaticNumber);
